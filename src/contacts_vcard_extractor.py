...
def parse_vcard_line(file_line: str) -> Dict:
    ...
    matching_key = next(
        (key for key in vcf_field_parser.SIMPLE_KEYS if file_line.startswith(key)),
        None
    )
    if matching_key:
...
def _parse_multiline_multimedia(vcf_file_lines: List[str], start_line: int) -> Tuple[str, int]:
    """
